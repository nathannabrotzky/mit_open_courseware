...
class WeightedEdge(Edge):
    def __init__(self:WeightedEdge, src:str, dest:str, total_distance:int, outdoor_distance:int):
        super().__init__(src, dest)
        self.total_distance:int = int(total_distance)
        self.outdoor_distance:int = int(outdoor_distance)

    def get_total_distance(self:WeightedEdge):
        return self.total_distance

    def get_outdoor_distance(self:WeightedEdge):
...
class Digraph(object):
    ...
    def __init__(self:Digraph):
        self.nodes:set = set([])
        self.edges:dict[Node,list[WeightedEdge]] = {}  # must be a dict of Node -> list of edges

    def __str__(self:Digraph):
        edge_strs:list[str] = []
        for edges in self.edges.values():
            for edge in edges:
                edge_strs.append(str(edge))
        edge_strs:list[str] = sorted(edge_strs)  # sort alphabetically
        return '\n'.join(edge_strs)  # concat edge_strs with "\n"s between them

    def get_edges_for_node(self:Digraph, node:str):
        return self.edges[Node(node)]

    def has_node(self:Digraph, node:str):
        return Node(node) in self.nodes

    def add_node(self:Digraph, node:Node):
    ...
    def add_edge(self:Digraph, edge:WeightedEdge):
        if Node(edge.src) not in self.nodes or Node(edge.dest) not in self.nodes:
            raise ValueError("missing a node associated with the edge")
        else:
            self.edges[Node(edge.src)].append(edge)

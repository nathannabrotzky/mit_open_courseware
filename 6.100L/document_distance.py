...
def get_frequencies(input_iterable:list[str]) -> dict[str,int]:
    """
    Args:
...
def get_letter_frequencies(word:str) -> dict[str,int]:
    """
    Args:
...
def calculate_similarity_score(freq_dict1:dict[str,int], freq_dict2:dict[str,int]) -> float:
    """
    ...
    diff_tot:float = 0
    all_tot:float = 0
    keys = set(freq_dict1.keys()) | set(freq_dict2.keys())
    for key in keys:
        if key in freq_dict1 and key in freq_dict2:
            diff_tot += abs(freq_dict1[key] - freq_dict2[key])
            all_tot += freq_dict1[key] + freq_dict2[key]
        elif key in freq_dict1:
            diff_tot += freq_dict1[key]
            all_tot += freq_dict1[key]
        elif key in freq_dict2:
            diff_tot += freq_dict2[key]
            all_tot += freq_dict2[key]
    return round(1 - (diff_tot/all_tot), 2)


### Problem 4: Most Frequent Word(s) ###
def get_most_frequent_words(freq_dict1:dict[str,int], freq_dict2:dict[str,int]) -> list[str]:
    """
    The keys of dict1 and dict2 are all lowercase,
    you will NOT need to worry about case sensitivity.
    ...
    """
    words = set(freq_dict1.keys())|set(freq_dict2.keys())
    merged_dict = {word: freq_dict1.get(word,0) +
                   freq_dict2.get(word,0) for word in words}
    result_list:list[str] = []
    for k, v in merged_dict.items():
        if (len(result_list) == 0 or
            (k not in result_list and
             v == merged_dict[result_list[0]])):
            result_list.append(k)
        elif k not in result_list and v > merged_dict[result_list[0]]:
            result_list = [k]
    return result_list
...
def get_tf(file_path:str) -> dict[str,float]:
    ...
    * Think about how we can use get_frequencies from earlier
    """
    text_file:str = load_file(file_path)
    text_list:list[str] = text_to_list(text_file)
    freq_dict:dict[str,int] = get_frequencies(text_list)
    tf_dict:dict[str,float] = {}
    for k, v in freq_dict.items():
        tf_dict[k] = v / len(text_list)
    return tf_dict
    ...
    for file in file_paths:
        ...
        for word in set(text_list):
            if word in idf_dict:
                ...
                idf_dict[word] = 1
    return {key: math.log10(len(file_paths)/value) for key, value in idf_dict.items()}
        

def get_tfidf(tf_file_path:str, idf_file_paths:list[str]):
    """
        Args:
    ...
    tf_idf_dict:dict[str,float] = {key: value * idf_dict[key] for key, value in tf_dict.items()}
    return sorted(tf_idf_dict.items(), key=lambda item: (item[1],item[0]))
